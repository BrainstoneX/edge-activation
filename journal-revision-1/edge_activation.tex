...
Nash-Williams \cite{Nash-Williams1961} considered \lasse{the optimization problem of packing as many spanning trees as possible into the graph, such that for every edge $e \in E$, it gets used in at most $w(e)$ spanning trees. For example, consider \cref{fig_spt_example}, where $G$ is a complete grap on three vertices, and $w(e) = 2$ for all edges. The optimal solution packs three spanning trees. It is not possible to pack more spanning trees, because every edge is already used twice.} 
%%%%%%%%%%%%%%%%%%%%%%%
\begin{figure}[htpb]
\centering
...
\lasse{\paragraph{Connections to other areas.}
...
This includes for example delay-tolerant networks where information links often fail, like satellite networks, vehicular or passenger networks.
 ...
 This class was for example used to always guarantee progression of a broadcast operation \cite{o2005information}, or to enable consensus in a TVG \cite{kuhn2011coordinated}. Our problem of non-preemptive tree packing can therefore be understood as scheduling the edges in a dynamic network in such a way that the resulting time-varying graph is 1-connected for the maximum possible time.}
...
By Lemma~\ref{C}, we may assume that in a feasible schedule of length~$3$ the 
...
respect to various parameters. \lasse{As the problem is already NP-hard even for very highly restricted graphs, considering all the usual parameters from parameterized complexity do not result in efficient algorithms. Therefore we can only provide algorithms for parameters which are in some way non-standard.}
