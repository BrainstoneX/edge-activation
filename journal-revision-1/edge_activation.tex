...
$E^\sigma_t$ and $G^\sigma_t$. 
 ...
 \lasse{We clarify two points in this definition: 
First, this set of time slots does not encessarily need to be a set of consecutive time slots. Formally, if $C(\sigma) = \set{t : G^\sigma_t \text{ is connected}}$ denotes the set of time slots where the graph is connected by schedule $\sigma$, then $\ntp(\sigma) = |C(\sigma)|$. Second, there can be of course a time slot $t$, where $G^\sigma_t$ is connected and contain more edges than a spanning tree, that is, the set of active edges contains a cycle. By our definition, such a time slot also increases $\ntp(\sigma)$ by one.
...
\begin{proof}
\lasse{
The algorithm selects an arbitrary vertex $x \in V(H')$ and constructs the new graph $H$ in the following way:
}
\end{proof}
%%%%%%%%%%%%%%%%%
